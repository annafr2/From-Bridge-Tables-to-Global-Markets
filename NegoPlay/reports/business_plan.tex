...
A B2B AI-Powered Negotiation Intelligence Platform\par}
\rule{\textwidth}{1.5pt}\\[1.5cm]

\begin{tcolorbox}[colback=accentblue!10, colframe=accentblue, boxrule=1pt]
\centering\large
\textbf{Tagline:} \emph{``Know your counterpart before you walk into the room.''}
\end{tcolorbox}
...
\section{The four profiles (hypothesis -- confirmed by Stage 1)}

\begin{table}[H]
\centering\small
\caption{Expected decision profiles discovered from bridge data.}
\begin{tabularx}{\textwidth}{l X X}
\toprule
\textbf{Profile} & \textbf{Core traits} & \textbf{In negotiation} \\
\midrule
Slam Hunter    & Aggressive, high risk tolerance, calculated upside bets
               & Opens high, concedes little, walks away fast if value is low \\
Insurance Player & Risk-averse, consistent, values certainty over upside
               & Prefers small guaranteed gains, long close cycles, many contingencies \\
Bluffer        & Aggressive opening but inconsistent follow-through
               & Signals high value but folds under pressure \\
Doubler        & Defensive-aggressive, challenges opponents' claims
               & Rejects initial offers almost always, uses anchoring counter-offers \\
\bottomrule
\end{tabularx}
\end{table}
...
The core cost driver is LLM token consumption. Each negotiation profile
analysis consumes approximately 5{,}000 tokens using Gemini Flash 2.0.

\begin{table}[H]
\centering\small
\caption{Unit economics per profile analysis.}
\begin{tabular}{lr}
\toprule
\textbf{Item}                        & \textbf{Cost} \\
\midrule
Gemini Flash 2.0 output tokens (5K)  & \$0.0015 \\
Gemini Flash 2.0 input tokens (3K)   & \$0.0002 \\
Infrastructure (hosting, storage)     & \$0.005  \\
\midrule
\textbf{Total cost per analysis}      & \textbf{\$0.007} \\
\textbf{Revenue per analysis (Pro)}   & \textbf{\$0.245} (200 calls / \$49) \\
\textbf{Gross margin per analysis}    & \textbf{97\,\%} \\
\bottomrule
\end{tabular}
\end{table}

\section{Token economics summary}

\begin{table}[H]
\centering\small
\caption{Token economics across the four pipeline stages.}
\begin{tabularx}{\textwidth}{l r r X}
\toprule
\textbf{Stage} & \textbf{Tokens/call} & \textbf{Cost/call} & \textbf{Notes} \\
\midrule
Stage 2 -- Skill extraction   & $\sim$8{,}000 & \$0.003  & One-time per cluster (amortized) \\
Stage 3 -- Agent prompt       & $\sim$2{,}000 & \$0.0006 & System prompt cached \\
Stage 4 -- Bridge simulation  & $\sim$3{,}000 & \$0.001  & Per bid turn \\
Stage 4 -- Negotiation sim    & $\sim$5{,}000 & \$0.0015 & Per negotiation round \\
\midrule
\textbf{Full pipeline (one user analysis)} & \textbf{$\sim$18{,}000} & \textbf{\$0.006} & End-to-end \\
\bottomrule
\end{tabularx}
\end{table}

\begin{keybox}[Cost advantage of Gemini Flash 2.0]
At \$0.30 per million output tokens, Gemini Flash 2.0 is approximately
10$\times$ cheaper than GPT-4o for equivalent throughput. The entire
NegoPlay MVP (development + testing) runs for under \$15 in LLM costs.
Claude (Anthropic) and GPT are used selectively for cross-model
validation runs to strengthen publishability, not for production traffic.
\end{keybox}
...
LLM API costs (Gemini, 97\,\% GM) & \$353         & \$2{,}376       & \$14{,}400 \\
