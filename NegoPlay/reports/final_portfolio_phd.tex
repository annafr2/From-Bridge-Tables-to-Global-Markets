...
remarkably hard to study: real negotiations are private, their outcomes are
...
NegoPlay offers early, fully reproducible evidence that decision-making
...
very subject of this project. Google Gemini (Flash~2.x) is the default
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Profile discovery (ML).} 149{,}208 EuroBridge records
        ...
        a smooth \emph{continuum}, not separable blobs. This is a substantive
...
\textbf{cross-domain behavioural consistency} (CDBC), or equivalently
\emph{dispositional policy alignment}: the degree to which the decision
...
environment.\footnote{We deliberately avoid the bare term \emph{style transfer}.
...
structured interaction and carry it into a different setting is supported by
...
lighter-weight route to the same kind of cross-context consistency.

\section{Profiles as behavioural archetypes}
The low silhouette score is itself a result: elite players do not fall into
disjoint clusters but populate a smooth, near-optimal behavioural continuum.
...
principled lower bound that separates the influence of the game's rules from
genuine skill.
...
\textbf{Cross-domain consistency.} With $K=5$ profiles, dispositional policy
alignment is the Spearman rank correlation between the bridge-aggression
...
ZI-C baseline test materially changed this project's measurements.
