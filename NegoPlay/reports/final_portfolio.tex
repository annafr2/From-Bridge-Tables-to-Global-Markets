...
simulations, and per-call cost logging ($4{,}644$ logged calls cost
${\approx}\$1.69$; total development spend, including pre-logging
experimentation and retried runs, ${\approx}30$ ILS / ${\approx}\$8$).
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Profile discovery (ML).} 149{,}208 EuroBridge records
        ...
        a smooth \emph{continuum}, not separable blobs. We therefore assign
        profiles by \textbf{extreme-percentile thresholding} (top $P\%$ on a
        profile's defining axis) gated by a one-sided binomial significance
        test ($p<0.05$ versus the population baseline, \texttt{scipy.stats.\allowbreak binomtest})
...
(a \emph{par-proxy}: it scored a bid by how close its contract level sat to a
heuristic ``par'' derived from the partnership's high-card points, rewarding
sensible-looking aggression but never checking whether the contract would
actually succeed) failed this test: a random ``monkey'' bidder
\emph{outscored} every expert profile (0.54 vs.\ 0.25--0.51), because random
bids still land near the heuristic par often enough. Re-scoring each contract
by whether it \emph{actually makes} under double-dummy (perfect-play)
evaluation of the real 52 cards collapses the monkey to last place (0.10):
...
concedes IMPs to the field). On this scale the random monkey sits at
...
together with a counter-price). Because the LLM occasionally proposes a number
outside the deal's plausible band, that price is \emph{clamped} to the
scenario's legal range, i.e.\ forced to lie between the seller's floor and its
opening ask, so a malformed reply cannot corrupt the downstream
